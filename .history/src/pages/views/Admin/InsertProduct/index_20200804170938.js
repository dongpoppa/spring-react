...
const InsertProduct = (props) => {
  ...
  let lastId = props.games[props.games.length - 1].id;
  const [selected, setSelected] = useState([]);
  const options = props.categories;

  const onSubmitHandle = (data) => {

    const uploadTask = storage.ref(`games_image/${getImageName.name}`).put(getImageName);
  ...
  const [getImage, setImageName] = useState(null);
