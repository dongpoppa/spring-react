  ...
  const addToCart = (e,game) => {
    ...
    if(game.quantity !== 0){
      if(props.cartList){
        ...
        props.addCart(cartDetail);
      }else{
        if(props.cartList.filter(cartDetail.game.id !== game.id)){
            const cartDetail = {
              game : game,
              quantity : 1
            }
          ...
          props.cartList.filter(cartDetail.game.id === game.id).map(cartDetail => {
            props.updateCart(cartDetail,1);
          })
        }
      }
